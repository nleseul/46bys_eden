...
class DisplayWindow:
    def __init__(self, master):
        ...
        self.master.bind('<Down>', self.__next_page)
        self.master.bind('<Up>', self.__prev_page)
        self.master.bind('<Right>', self.__next_entry)
        self.master.bind('<Left>', self.__prev_entry)
    ...
    def __draw(self):
        ...
        for typeset_line in range(0, self.display_height):
            start_index = (self.current_page * self.display_height + typeset_line) * self.display_width
    ...
    def __next_page(self, event):
        if not self.__is_last_page():
            self.current_page += 1
            
            self.__typeset()
        
    def __prev_page(self, event):
        if self.current_page > 0:
            self.current_page -= 1
            
            self.__typeset()
            
    def __next_entry(self, event):
